...
- **Confidence + abstention**: calibrate confidence against a held-out set (e.g., conformal prediction) and abstain to human review when uncertainty exceeds the risk budget or the clause is out-of-playbook ([arthur.ai](https://www.arthur.ai/column/what-to-look-for-ai-observability-platform-2026)). **Implemented (deterministic subset):** `attach_confidence` in `generate_dashboard.py` derives a per-risk 0-1 confidence and a `reviewRequired` flag from observable signals (citation verification, source-clause linkage, playbook coverage, field completeness, mandatory escalation) plus an aggregate `reliability` summary; the dashboard surfaces both. **Calibration harness (deterministic):** the `calibration/` package + [`confidence_calibration.md`](confidence_calibration.md) provide the labeled-dataset schema, calibration metrics (Brier, ECE, MCE, reliability diagram), selective-prediction metrics, a histogram-binning calibrator, an evaluation CLI, and a pytest quality gate. Large-scale human/LLM labeling, isotonic/conformal calibrators, and wiring the calibrated score into the auto-accept threshold remain future work (needs the model/eval infrastructure in P-E).
